  ...
  public virtual void Jump(bool ignoreGrounded = false, float? overrideImpulse = null) {
    if (IsGrounded || ignoreGrounded) {
      ...
      newVel.y += overrideImpulse ?? jumpImpulse;
            ...
            {
                newVel.y = Mathf.Sqrt((jumpImpulse / 4f) * -2f * gravity / playerScript.umbrellaFloat);
            }
            else
            {
                newVel.y = Mathf.Sqrt(jumpImpulse * -2f * gravity );
            }
    ...
    public void UmbrellaFall()
    {
        ...
        newVel.y = Mathf.Sqrt(jumpImpulse / playerScript.umbrellaFloat);
    ...
    if (IsGrounded && moveVelocity.y < 0) {
      moveVelocity.y = 0f;
        ...
        moveVelocity.y += (gravity / playerScript.umbrellaFloat) * Runner.DeltaTime;
